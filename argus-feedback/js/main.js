...
// Debounce-function:
	...
	$('.sbmt-btn').on('click', function (e) {
		...
		if (validateForm(form)) {
			...
			// var files = $(this).parents('.form').find('input[name="file"]').val();
			// sendMultipleFiles(name, phone, email, files);
			// sendForm(form);
		}
	});
	...
	$('.bot-send-btn').on('click', function (e) {
		...
		var fileListName = ['Cписок документов для регистрации работодателя.pdf'];
		var form = $(this).parents('.form');
		var email = $(this).parents('.form').find('input[name="email"]').val();
	...
	function sendMultipleFiles(name, phone, email, files) {
		$.ajax({
			type: 'POST',
			url: 'sendFiles.php',
			data: 'email=' + email + '&files=' + files + '&phone=' + phone + '&name=' + name
		});
	}
